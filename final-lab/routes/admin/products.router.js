...
//sorting
router.get("/admin/products/:page?/:sortby?", async (req, res) => {
  ...
  if ((sortby == "price" || sortby == "Quantity") && (sortby!=="All")){
      ...
      let totalRecords = await Product.countDocuments();
      let totalPages = Math.ceil(totalRecords / pageSize);
      res.render("pages/admin/product", {layout: "admin-layout.ejs",products,page,totalRecords,totalPages,Productshown,pageSize});
  }
  else{
      let page = req.params.page ? Number(req.params.page) : 1;
      let pageSize = 8;
...
router.get('/admin/orders', async (req, res) => {
  ...
  res.render('pages/admin/orderhistory', { orders, layout :"admin-layout.ejs" });
});
